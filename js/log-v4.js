...
(() => {
    ...
    function attachVisitButtonToShrineDetail() {
        ...
        button.textContent = editingRecord ? '⛩ この神社に変更する' : '📖 この神社を参拝記録する';
    ...
    function shrineFromMaster(jinjaId, fallbackRecord = null) {
        const jinja = Array.isArray(window.jinjaData)
        ...
        if (fallbackRecord && fallbackRecord.shrineId) {
            return {
                id: fallbackRecord.shrineId,
                name: fallbackRecord.shrineName || fallbackRecord.shrineId,
                yomi: fallbackRecord.shrineYomi || '',
                address: fallbackRecord.shrineAddress || '',
                province: fallbackRecord.shrineProvince || ''
            };
        }
        return null;
    }
    ...
    function openVisitFormForShrine(jinjaId) {
        const shrine = shrineFromMaster(jinjaId);
        if (!shrine) return;
        ...
        const message = editingRecord
            ? `${selectedShrine.name} に変更しました。内容を確認して「変更を保存」してください。`
            : `${selectedShrine.name} の参拝記録を入力できます。`;
    ...
    function renderSelectedShrine() {
        ...
        if (!selectedShrine) {
            ...
            if (metaEl) metaEl.textContent = editingRecord
                ? 'この旧記録は神社未設定です。神社マップから選び直してください。'
                : '神社詳細の「この神社を参拝記録する」から開始します。';
            if (changeButton) changeButton.textContent = editingRecord ? '神社マップで設定する' : '神社マップで選ぶ';
            return;
        }
        ...
        if (changeButton) changeButton.textContent = editingRecord ? '神社を変更する' : '神社マップで選び直す';
    }
    ...
    function initEditControls() {
        ...
        if (!saveButton || document.getElementById('visit-edit-cancel')) return;

        const cancelButton = document.createElement('button');
        cancelButton.type = 'button';
        cancelButton.id = 'visit-edit-cancel';
    ...
    function setEditingUi(isEditing) {
        const saveButton = document.getElementById('visit-save');
        const cancelButton = document.getElementById('visit-edit-cancel');
        if (saveButton) saveButton.textContent = isEditing ? '変更を保存' : 'この参拝記録を保存';
        if (cancelButton) cancelButton.hidden = !isEditing;
        const card = document.getElementById('visit-shrine-card');
        if (card) card.classList.toggle('editing', isEditing);
    }
    ...
    function initPhotoInput() {
        ...
        clearButton.addEventListener('click', () => {
            input.value = '';
            clearSelectedPhoto();
            setStatus(editingRecord ? '御朱印写真を外して保存できます。' : '御朱印写真なしで保存できます。', 'ready');
        });
    }

    function showPhotoPreview(blob, clearLabel) {
        const preview = document.getElementById('photo-preview');
        const clearButton = document.getElementById('photo-clear');
        ...
        if (clearButton) {
            clearButton.textContent = clearLabel || '写真を選び直す';
    ...
    function clearSelectedPhoto() {
        ...
        if (clearButton) {
            clearButton.textContent = '写真を選び直す';
    ...
    async function handleSubmit(event) {
        ...
        const record = {
            ...
            photoName: selectedPhoto ? (selectedPhoto.name || editingRecord?.photoName || 'goshuin-photo') : '',
            photoType: selectedPhoto ? (selectedPhoto.type || editingRecord?.photoType || 'image/*') : '',
            createdAt: editingRecord?.createdAt || now,
            updatedAt: wasEditing ? now : (editingRecord?.updatedAt || '')
        };

        try {
            saveButton.disabled = true;
            saveButton.textContent = wasEditing ? '更新中…' : '保存中…';
            ...
            await renderVisitLogs();
            setStatus(
                wasEditing ? `${record.shrineName} の参拝記録を更新しました。` : `${record.shrineName} の参拝記録を保存しました。`,
                'success'
            );
        } catch (error) {
            ...
            saveButton.textContent = editingRecord ? '変更を保存' : 'この参拝記録を保存';
        }
    }
    ...
    function cancelEditing() {
        ...
        const dateInput = document.getElementById('visit-date');
        const memoInput = document.getElementById('visit-memo');
        const photoInput = document.getElementById('goshuin-photo');
        if (dateInput) {
            dateInput.value = '';
            setToday();
        }
        ...
        clearSelectedPhoto();
        setStatus('編集をキャンセルしました。', 'ready');
    }

    function startEditing(record) {
        editingRecord = record;
        selectedShrine = record.shrineId ? shrineFromMaster(record.shrineId, record) : null;
        renderSelectedShrine();

        const dateInput = document.getElementById('visit-date');
        const memoInput = document.getElementById('visit-memo');
        const photoInput = document.getElementById('goshuin-photo');
        if (dateInput) dateInput.value = record.visitDate || '';
        if (memoInput) memoInput.value = record.memo || '';
        if (photoInput) photoInput.value = '';

        clearSelectedPhoto();
        if (record.photo instanceof Blob) {
            selectedPhoto = record.photo;
            showPhotoPreview(record.photo, '写真を外す');
        }
        ...
        const logTab = document.querySelector('.nav-tab[data-view="log-view"]');
        if (logTab) logTab.click();
        setStatus('参拝記録を編集中です。内容を直して「変更を保存」してください。', 'ready');
        document.querySelector('.visit-entry-panel')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }
    ...
    function renderShrineGroups(records, list) {
        ...
        records.forEach(record => {
            ...
            if (!groups.has(key)) {
                groups.set(key, { name, sortKey: record.shrineYomi || name, records: [] });
            }
    ...
    function renderGodGroups(records, list) {
        const groups = new Map();
        records.forEach(record => {
            getGodsForRecord(record).forEach(god => {
                if (!groups.has(god.id)) {
                    groups.set(god.id, { name: god.name, yomi: god.yomi || god.name, records: [] });
                }
    ...
    function getGodsForRecord(record) {
        ...
        return ids.map(id => {
            ...
            return { id, name: god?.name || id, yomi: god?.yomi || god?.name || id };
        });
    }
    ...
    function initModal() {
        ...
        const dialog = modal?.querySelector('.log-detail-dialog');
        if (!modal || !close || !dialog) return;

        let actions = document.getElementById('log-detail-actions');
        if (!actions) {
            actions = document.createElement('div');
            actions.id = 'log-detail-actions';

            const editButton = document.createElement('button');
            editButton.type = 'button';
            editButton.id = 'log-detail-edit';
            editButton.textContent = '✏️ 編集';
            editButton.addEventListener('click', () => {
                if (currentDetailRecord) startEditing(currentDetailRecord);
            });

            const deleteButton = document.createElement('button');
            deleteButton.type = 'button';
            deleteButton.id = 'log-detail-delete';
            deleteButton.textContent = '🗑 削除';
            deleteButton.addEventListener('click', handleDeleteCurrentRecord);
    ...
    async function handleDeleteCurrentRecord() {
        ...
        const label = `${getShrineName(record)}（${formatDate(record.visitDate)}）`;
        if (!window.confirm(`${label} の参拝記録を削除しますか？\nこの操作は元に戻せません。`)) return;

        try {
            await deleteRecord(record.id);
            if (editingRecord?.id === record.id) cancelEditing();
            closeDetail();
            await renderVisitLogs();
            setStatus(`${label} を削除しました。`, 'success');
        } catch (error) {
            console.error('参拝記録の削除に失敗しました:', error);
