...
UA = {"User-Agent":"kamisama-db-research-builder/3.0 (+https://github.com/Phantom-Theatre88/kamisama-db)"}
...
def with_page(url,page):
    p=urlsplit(url); q=dict(parse_qsl(p.query,keep_blank_values=True)); q["page"]=str(page); q["sort_field"]="f138"; q["sort_type"]="asc"; q["search_type"]="keyword"; q["btn_list_type"]="yes"
...
def collect(start_url):
    ...
    for page in range(1,pages+1):
        html=first if page==1 else fetch(with_page(start_url,page))
        current=list_links(html); new=0
        for item in current:
        ...
        time.sleep(.1)
    if declared and len(all_links)<declared*0.98: raise RuntimeError(f"list collection incomplete: {len(all_links)}/{declared}")
...
def in_existing(name,lat,lng,base):
    nk=norm(name)
    for n,a,b in base:
        if n==nk and a is not None and b is not None and distance(lat,lng,a,b)<=2: return True
    return False

def parse_modern(item):
    url,data_id=item
    try:
        f,soup=fields(fetch(url))
        name=clean(f.get("団体名") or f.get("+神社名") or f.get("神社名"))
        if not name:
            h=soup.find(["h1","h2"]); name=clean(h.get_text(" ",strip=True)) if h else ""
        yomi=clean(f.get("+神社名よみ") or f.get("神社名よみ")); pref=clean(f.get("都道府県名（上級行政区画）") or f.get("都道府県名、上級行政区画")); province=clean(f.get("旧国名")); addr=clean(f.get("住所")); note=clean(f.get("+備考") or f.get("備考"))
        if pref and addr and not addr.startswith(pref): addr=pref+addr
        coord=' '.join([f.get("緯度/経度 latitude/longitude",''),f.get("緯度経度",''),soup.get_text(' ',strip=True)])
        lat,lng=dms(coord)
...
def numbered(f,prefix):
    pat=re.compile(rf"^\+?{re.escape(prefix)}（(\d+)）$"); out=[]
    for k,v in f.items():
        m=pat.match(k)
...
def parse_shiki(item):
    url,data_id=item
    try:
        f,soup=fields(fetch(url)); province=clean(f.get("国名") or f.get("旧国名")); county=clean(f.get("旧郡名")); st=clean(f.get("名神大社・大社・小社") or f.get("社格")) or "式内社"; out=[]
        for idx,raw in numbered(f,"現社名など"):
            cand="論社" in raw; name=clean(re.sub(r"^[（(]\s*論社\s*[）)]\s*","",raw)); coord=(f.get(f"+現社名など（{idx}）緯度経度") or f.get(f"現社名など（{idx}）緯度経度") or "")+' '+(f.get(f"+現社名など（{idx}）リンク") or f.get(f"現社名など（{idx}）リンク") or ""); lat,lng=dms(coord)
...
def main():
    ...
    report={"modern_declared":modern_declared,"modern_links":len(modern_links),"shikinai_declared":shiki_declared,"shikinai_links":len(shiki_links),"curated_master":len(base),"modern_generated_before_cross_dedupe":len(modern),"tier2_generated_rows":len(rows),"total_app_rows":len(base)+len(rows),"status":"complete" if len(rows)>=2000 else "insufficient","dedupe":"normalized name + coordinate proximity"}
